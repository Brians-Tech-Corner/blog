...
Kubernetes requires swap to be completely disabled. Ubuntu's LVM installer creates a swap logical volume by default, so we need to disable it and remove it completely:
...
# Remove the swap logical volume (Ubuntu creates this by default)
sudo lvremove -y /dev/ubuntu-vg/swap_1
...
Before converting this VM into a reusable template:
