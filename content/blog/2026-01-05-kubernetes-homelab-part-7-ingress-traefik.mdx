...
Now that inbound traffic is solved, you can start deploying “real” apps cleanly.

Next post ideas (pick one):

- **Local DNS for `*.k8s.local`** (Pi-hole/Unifi DNS/CoreDNS overrides)
- **MetalLB** to enable `LoadBalancer` services
- **First real workload deployment** (an app + gateway route + PVC)

➡️ **Next:** *Kubernetes on Proxmox – DNS + MetalLB (Optional) or First App Deployment*
