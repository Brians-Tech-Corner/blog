...
Here's what it actually looks like in Claude Desktop — tools being called, results coming back, and Claude reasoning through the output in real time:

<ClickableImage
  src="/post-images/homelab-mcp-claude-ss.png"
  alt="Claude Desktop using the homelab MCP server to query a Kubernetes cluster"
  caption="Claude Desktop calling the homelab MCP tools directly — no copy-pasting required"
  width={1200}
  height={800}
/>

After confirming the tool call, Claude fetches the logs and immediately starts diagnosing what's wrong — no terminal switching, no copy-pasting:

<ClickableImage
  src="/post-images/homelab-mcp-claude-ss-logs.png"
  alt="Claude Desktop showing container logs and diagnosing the issue"
  caption="Claude pulling logs and pinpointing the root cause in one shot"
  width={1200}
  height={800}
/>
